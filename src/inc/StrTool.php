...
class StrTool
{
     ...
     * @param $type  integer  类型 [ 0:数字，1:数字+字母，2:汉字 ]
     * @return string 返回随机字符串
     */
    public static function random($length, $type=1)
    {
        $charset = self::$code_map;
        $string = '';
        if($type == 0){
            $l = mb_strlen($charset[$type], "UTF-8");
        }else{
            $l = strlen($charset[$type]);
        }

        for ($i=0;$i<$length;$i++){
            $string .= $charset[$type][mt_rand(0,$l)];
        }
