...
class DopenWechat
{
    ...
    function __construct($t)
    {
        ...
        //dd($b);
        if (stripos($b->body, $this->t['name']) == false) {
            $b = $this->Dhttp->Dhttp("https://open.weixin.qq.com/cgi-bin/login", ['account' => $this->t['name'], 'passwd' => md5($this->t['dpassword']), 'lang' => 'zh_CN', 'f' => 'json', 'ajax' => 1, 'key' => '1'], 'https://open.weixin.qq.com/');

            $back = json_decode($b->body, true);
            if ($back['nick_name'] == $this->t['name']) {
                $this->t['token'] = $back['base_resp']['token'];
                DB::table('open_accounts')->where(['id' => $this->t['id']])->update(["token" => $this->t['token']]);

            }
        } else {
            $login_token = DB::table('open_accounts')->where(['id' => $this->t['id']])->first();
            //$login_token = M("owechat","deep_")->where(['id'=>$this->t['id']])->find(); 
            $this->t['token'] = $login_token->token;
        }

        /*if (stripos($b->body, $this->t['name'])) {
            $b = $this->Dhttp->Dhttp("https://open.weixin.qq.com/cgi-bin/login", ['account' => $this->t['name'], 'passwd' => md5($this->t['dpassword']), 'lang' => 'zh_CN', 'f' => 'json', 'ajax' => 1, 'key' => '1'], 'https://open.weixin.qq.com/');
    ...
    public function sqdisanfang($d = "")
    {
        ...
        // 开始提交第三方平台资料
        $nwxlist = DB::table("open_platforms")->insertGetId(['name' => "---"]);
        ...
        if (isset($backdata['base_resp']) && $backdata['base_resp']['err_msg'] != "ok") {
            if ($backdata['base_resp']['ret'] == "1054") {
                ...
                return $res_data = ['status' => 0, 'msg' => "访问不到域名指向下的文件,如果确认已经生效可以多点击几次"];
            }
        }
        ...
        //$d['appid'] = $oname."简介";
        //$nwxlist = D("open_platforms")->add($d);
        ...
        if (isset($back['base_resp']) && $back['base_resp']['err_msg'] == "ok" && $back['err_msg'] == "ok") {
            ...
            $d['token']=$d['auth_token'];
            $d['auth_token'] = $this->t['token'];
            foreach ($appidmap as $key => $value) {
                if ($value['name'] == $oname) {
                    $save_data['appid'] = $value['appid'];
                    $save_data['available_share'] = isset($urlmap[2]) ?  $urlmap[2] : "";
                    $save_data['available_py'] = isset($urlmap[1]) ?  $urlmap[1] : "";
                    $save_data['available'] = isset($urlmap[0]) ?  $urlmap[0] : "";
                    $save_data['open_account_id'] = $this->t['id'];
                    $save_data['iplist'] = $this->t['iplist'];
                    $save_data['gzhlist'] = $this->t['gzhlist'];
                    $save_data['domain'] = $d['sns_domain'];
                    $save_data['zhanghao'] = $this->t['name'];
                    $save_data['mima'] = $this->t['dpassword'];
                    $d['enable'] = 0;
                    $n = array_merge($d, $save_data);
                    //dd($n);
                    //var_dump($save_data);die;
                    //D("open_platforms")->where(['id'=>$nwxlist])->save($n);
                    DB::table('open_platforms')->where(['id' => $nwxlist])->update($n);
                }
            }
            //echo sprintf("第三方平台申请成功！申请名称为：%s,请登录复制APPSECRET", $oname);die;

            return $res_data = ['status' => $nwxlist, 'msg' => "第三方平台申请成功！申请名称为：{$oname},请登录复制APPSECRET"];
        } else {
            //M("owechat","deep_")->where(['id'=>$nwxlist])->delete();
            //D("open_platforms")->where(['id'=>$nwxlist])->delete();
            DB::table('open_platforms')->where(['id' => $nwxlist])->delete();
            //echo "第三方平台申请失败！".json_encode($back); die;

            return $res_data = ['status' => 0, 'msg' => "第三方平台申请失败！" . json_encode($back)];
        }
    }
    ...
    public function deletedsf($appid = "")
    {
        ...
        if ($backdata['base_resp']['err_msg'] != "ok") {
            //echo "未能在公众平台删除，请登录手动删除";
           // die;
            return $res_data = ['status' => 0, 'msg' => "未能在公众平台删除，请登录手动删除！" ];
        }else{
            return $res_data = ['status' => 1, 'msg' => "删除成功" ];
        }
    }
    ...
    public function getqrcode($appid)
    {
        ...
        $refer = 'https://open.weixin.qq.com/wxaopen/serviceprovider/'.$appid.'?token='.$this->t['token'];
        $b = $this->Dhttp->Dhttp($url,$data,$refer);
        return $b->body;
    }


    public function qrcode($appid)
    {
        //dd(public_path('tongji/qrcode.jpg'));
        if(is_file(public_path('tongji/qrcode.jpg'))){
            unlink(public_path('tongji/qrcode.jpg'));

        }

        $token = $this->t['token'];
        $qrcheck_ticket = json_decode($this->getqrcode($appid,$token),true)['qrcheck_ticket'];
        $url = 'https://open.weixin.qq.com/cgi-bin/authqr?action=getqrcode&qrcheck_ticket='.$qrcheck_ticket.'&size120&token='.$token;

        $refer = 'https://open.weixin.qq.com/wxaopen/serviceprovider/'.$appid.'?token='.$token;

        $res = $this->Dhttp->DownloadImg($url,'qrcode.jpg',$refer);

        if($res){
            echo '<img src="http://'.$_SERVER['HTTP_HOST'].'/tongji/qrcode.jpg'.'">';
    ...
    public function code_ask($appid)
    {
        $token = $this->t['token'];
        $qrcheck_ticket = json_decode($this->getqrcode($appid,$token),true)['qrcheck_ticket'];

        $url = 'https://open.weixin.qq.com/cgi-bin/authqr?action=ask&qrcheck_ticket='.$qrcheck_ticket.'&key=ask&token='.$token.'&f=json';
        ...
        for ($i=0;$i<100;$i++){
            ...
            $b = $this->Dhttp->Dhttp($url,null,$refer);

            if(!empty(json_decode($b->body,true)['extra']['wx_id'])){
                //dump($b->body);
                $wx_id = json_decode($b->body,true)['extra']['wx_id'];
                echo '<h3>授权成功，授权微信号：'.$wx_id.'</h3>';
                break;
            }
    ...
    public function appdetail($appid)
    {
        ...
        for ($i=0;$i<50;$i++) {
            usleep(50000);
            $b = $this->Dhttp->Dhttp($url, $data, $refer);
            if(!empty(json_decode($b->body,true)['secret'])){
                // dump($b->body);
                $secret = json_decode($b->body,true)['secret'];
                break;
            }
        }
